...
const TOOLS = [
  {
    ...
    color: '#10A37F',
    featuresKo: [
      'GPTs를 통한 맞춤형 Agent 생성',
      'Code Interpreter로 데이터 분석 및 시각화',
      'DALL-E 기반 이미지 생성',
      'Web Browsing으로 실시간 정보 검색',
      'Advanced Data Analysis로 파일 처리',
    ],
    featuresEn: [
      'Custom Agent creation via GPTs',
      'Data analysis & visualization with Code Interpreter',
      'Image generation with DALL-E',
      'Real-time info search with Web Browsing',
      'File processing with Advanced Data Analysis',
    ],
    tipsKo: [
      '역할 부여 시 "당신은 ~분야 전문가입니다"로 시작하면 답변 품질이 향상됩니다',
      'Custom Instructions에 조직 컨텍스트를 저장하면 매번 배경 설명을 반복할 필요가 없습니다',
      'GPTs에 지식 파일을 업로드하면 조직 전용 Agent를 만들 수 있습니다',
      '"단계별로 생각하세요(Think step by step)" 프롬프트는 복잡한 문제에서 정확도를 높입니다',
      '출력 형식을 명확히 지정하세요: 표, 마크다운, 번호 목록 등',
    ],
    tipsEn: [
      'Start with "You are an expert in ~" for better response quality',
      'Save organizational context in Custom Instructions to avoid repeating background info',
      'Upload knowledge files to GPTs to create organization-specific Agents',
      '"Think step by step" prompt improves accuracy for complex problems',
      'Specify output format clearly: table, markdown, numbered list, etc.',
    ],
    promptsKo: [
  ...
  },
  {
    ...
    color: '#D97706',
    featuresKo: [
      'Projects로 프로젝트별 전용 컨텍스트 관리',
      '200K 토큰 컨텍스트로 긴 문서 전체 분석',
      'Artifacts를 통한 결과물 실시간 미리보기',
      '논리적 구조화와 체계적 검토에 뛰어난 성능',
      '코드 작성 및 수학적 추론 능력',
    ],
    featuresEn: [
      'Project-specific context management via Projects',
      'Full document analysis with 200K token context',
      'Real-time preview of outputs through Artifacts',
      'Excellent logical structuring and systematic review',
      'Code writing and mathematical reasoning capabilities',
    ],
    tipsKo: [
      'Claude Projects에 조직 문서와 가이드를 업로드하면 조직 맞춤형 답변을 받을 수 있습니다',
      '긴 보고서는 전체를 붙여넣고 분석을 요청하면 뛰어난 요약 품질을 보여줍니다',
      'Artifacts 기능을 활용하면 코드, 문서, 다이어그램 등을 인라인으로 미리 볼 수 있습니다',
      '복잡한 논리적 검토(계약서, 정책 문서 등)에서 특히 강점을 보입니다',
      '체계적인 XML 태그 구조로 프롬프트를 작성하면 더 정확한 결과를 얻을 수 있습니다',
    ],
    tipsEn: [
      'Upload organization docs and guides to Claude Projects for customized responses',
      'Paste entire long reports for analysis — Claude excels at summarization',
      'Use Artifacts to preview code, documents, and diagrams inline',
      'Particularly strong in complex logical reviews (contracts, policy documents)',
      'Structure prompts with XML tags for more precise results',
    ],
    promptsKo: [
  ...
  },
  {
    ...
    color: '#4285F4',
    featuresKo: [
      'Google 검색 통합으로 최신 정보 실시간 접근',
      'Google Workspace(Gmail, Docs, Drive) 연동',
      'Gems를 통한 맞춤형 AI 도우미 생성',
      '멀티모달 입력(이미지, PDF 분석) 지원',
      'Google 생태계와의 원활한 통합',
    ],
    featuresEn: [
      'Real-time access to latest info via Google Search',
      'Google Workspace (Gmail, Docs, Drive) integration',
      'Custom AI assistants via Gems',
      'Multimodal input (image, PDF analysis) support',
      'Seamless integration with Google ecosystem',
    ],
    tipsKo: [
      'Google Workspace와 연동하면 Gmail, Drive 파일을 기반으로 답변을 생성할 수 있습니다',
      '최신 뉴스나 실시간 데이터가 필요한 리서치에 특히 유용합니다',
      'Gems에 반복 업무 지침을 저장하면 매번 새로 프롬프트를 작성할 필요가 없습니다',
      '이미지를 업로드하여 차트나 그래프를 분석할 수 있습니다',
      '구글 독스에서 직접 Gemini를 호출하여 문서 작성을 가속할 수 있습니다',
    ],
    tipsEn: [
      'Link with Google Workspace to generate responses based on Gmail and Drive files',
      'Particularly useful for research requiring latest news or real-time data',
      'Save recurring task instructions in Gems to avoid rewriting prompts',
      'Upload images to analyze charts and graphs',
      'Call Gemini directly from Google Docs to accelerate document writing',
    ],
    promptsKo: [
  ...
  },
  {
    ...
    featuresKo: [
      '다중 소스 통합 검색 및 크로스체크',
      'Sparkpages로 리서치 결과 자동 구조화',
      '복잡한 주제에 대한 종합 보고서 생성',
      '실시간 웹 검색 기반 최신 정보 반영',
      '시각적으로 정리된 리서치 결과물 제공',
    ],
    featuresEn: [
      'Multi-source integrated search and cross-checking',
      'Auto-structured research results via Sparkpages',
      'Comprehensive report generation on complex topics',
      'Latest info based on real-time web search',
      'Visually organized research deliverables',
    ],
    tipsKo: [
      '복잡한 주제를 조사할 때 다른 AI 도구의 결과와 크로스체크용으로 활용하세요',
      'Sparkpages는 리서치 결과를 자동으로 구조화해주므로 보고서 초안으로 바로 활용할 수 있습니다',
      '산업 분석, 시장조사, 기술 동향 등 종합적 조사에 특히 효과적입니다',
      '검색 결과의 출처를 확인하여 신뢰성을 검증하세요',
      '여러 관점을 비교하는 질문을 하면 균형 잡힌 결과를 얻을 수 있습니다',
    ],
    tipsEn: [
      'Use for cross-checking results from other AI tools on complex topics',
      'Sparkpages auto-structures results — use directly as report drafts',
      'Particularly effective for industry analysis, market research, and tech trends',
      'Verify credibility by checking sources of search results',
      'Ask comparison questions for balanced perspectives',
    ],
    promptsKo: [
  ...
  },
  {
    ...
    featuresKo: [
      '모든 답변에 출처(URL) 자동 표시',
      '팔로업 질문을 통한 심층 탐색',
      'Collections로 리서치 주제별 정리',
      'Focus 모드로 학술/뉴스/소셜 등 소스 선택',
      '파일 업로드 기반 분석 지원',
    ],
    featuresEn: [
      'Auto-citation (URL) on all responses',
      'Deep exploration via follow-up questions',
      'Topic-based organization with Collections',
      'Source selection via Focus mode (Academic/News/Social)',
      'File upload-based analysis support',
    ],
    tipsKo: [
      'Focus 모드에서 "Academic"을 선택하면 학술 논문 기반 리서치가 가능합니다',
      '팔로업 질문을 연속으로 하면 주제를 점점 깊이 파고들 수 있습니다',
      'Collections 기능을 활용하면 프로젝트별로 리서치 히스토리를 관리할 수 있습니다',
      '출처가 명시되므로 보고서 작성 시 참고문헌 정리가 용이합니다',
      '"~에 대한 최신 연구 동향을 알려줘"와 같은 리서치 질문에 최적화되어 있습니다',
    ],
    tipsEn: [
      'Select "Academic" in Focus mode for research paper-based results',
      'Chain follow-up questions to progressively deepen topic exploration',
      'Use Collections to manage research history by project',
      'Citations make it easy to organize references for reports',
      'Optimized for research questions like "latest research trends on ~"',
    ],
    promptsKo: [
  ...
  },
  {
    ...
    featuresEn: [
      'Auto-convert text to diagrams',
      'Process flowchart generation',
      'Infographic-style visualization',
      'Auto-generate visuals for presentations',
      'Various layout and style options',
    ],
    tipsKo: [
      '다른 AI로 생성한 텍스트 결과물을 Napkin에 붙여넣으면 즉시 시각화할 수 있습니다',
      '워크플로우나 프로세스 설명을 단계별로 작성하면 깔끔한 플로우차트가 생성됩니다',
      '비교 분석 결과를 입력하면 비교 다이어그램을 자동으로 만들어줍니다',
      '발표자료 준비 시 핵심 포인트를 정리한 후 시각화하면 효과적입니다',
      'Agent 설계 캔버스의 결과물을 시각화하는 데 활용할 수 있습니다',
    ],
    tipsEn: [
      'Paste text outputs from other AIs into Napkin for instant visualization',
      'Write step-by-step workflow descriptions for clean flowcharts',
      'Input comparison analysis for auto-generated comparison diagrams',
      'Organize key points before visualization for effective presentations',
      'Use to visualize Agent design canvas outputs',
    ],
    promptsKo: [
